...
export function findChainTargets(
  ...
  const nearbyEnemies = allEnemies.filter(
    (enemy) =>
      enemy.id !== primary.id &&
      distance(enemy.position, primary.position) <= chainRangePixels
  );

  // Sort by distance from primary (closest first for chain effect)
  nearbyEnemies.sort(
    (a, b) =>
      distance(a.position, primary.position) -
      distance(b.position, primary.position)
  );
...
export function getEnemiesInSplash(
  ...
  return allEnemies.filter(
    (enemy) => distance(enemy.position, position) <= splashRadiusPixels
  );
}
