...
export function findPath(grid: CellState[][], start: Point, end: Point): Point[] {
  ...
  if (
    ...
    start.x < 0 || start.x >= grid[start.y].length ||
    end.y < 0 || end.y >= grid.length ||
    end.x < 0 || end.x >= grid[end.y].length
  ) {
    return [];
  }
...
export function wouldBlockPath(grid: CellState[][], proposedTower: Point): boolean {
  ...
  if (
    proposedTower.y < 0 || proposedTower.y >= grid.length ||
    proposedTower.x < 0 || proposedTower.x >= grid[proposedTower.y].length
  ) {
    return true;
  }
